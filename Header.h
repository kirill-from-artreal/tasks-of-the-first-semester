...
#include <vector>
#include <string>
#include <list> // queue тоже подойдет
...
struct FuncDB
{
	int id = 0;
	PFunc pfunc = NULL;
	string description = "Описание функции отсутствует.";
};
...
struct ItemDB
{
	...
	int funcID = -1;
	int menuID = -1;
	string title = "Заголовок пункта отсутствует.";
	string description = "Описание пункта отсутствует.";
};
...
struct MenuDB
{
	int id = 0;
	vector<int> itemID;
	string title = "Заголовок меню отсутствует.";
};
//========================================================
...
T check_input(string s)
{
    ...
    if (ss.fail())
        return ERR_INCORRECT_NUMBER; //std::cout << "Ошибка: введите корректное число\n"; //ss.clear(); //ss.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    else {
        std::string str;
        ss >> str;
        if (!str.empty())
            return ERR_INCORRECT_NUMBER_TYPE; //std::cout << "Ошибка: неверный числовой тип. Строка: "<< str <<"\n"; // ss.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
        return value;
    }
}
