...
int  task1(void) {
    ...
    double room_length = safe_input_double("Введите длину комнаты (м): ");
    double room_width = safe_input_double("Введите ширину комнаты (м): ");
    double room_height = safe_input_double("Введите высоту комнаты (м): ");
    double wallpaper_length = safe_input_double("Введите длину рулона обоев (м): ");
    double wallpaper_width = safe_input_double("Введите ширину рулона обоев (м): ");
    ...
    if (stripes_per_roll == 0) {
        std::cout << "Рулон слишком короткий для высоты стены" << std::endl;
        return 1; 
    }
    ...
    std::cout << "Необходимо рулонов: " << rolls_needed << std::endl;
    std::cout << "Остаток обоев в метрах: " << leftover_wallpaper << " м" << std::endl;
    std::cout << "Процент остатка: " << waste_percentage << "%" << std::endl;
